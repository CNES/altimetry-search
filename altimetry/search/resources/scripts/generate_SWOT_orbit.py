...
def extract_geometry(lon: numpy.ndarray, lat: numpy.ndarray) -> dict:
    ...
    left_lon,  left_lat  = _swath_polygon(lon, lat, x_orig, col_hi=nadir_idx - 1, col_lo=0)
    right_lon, right_lat = _swath_polygon(lon, lat, x_orig, col_hi=n_pixels - 1, col_lo=nadir_idx)
...
def get_average_timing(time, cycnums, cycle_start_map) -> dict:
    ...
    return {'ptime_ns': ptime, 'start': start, 'end': end,
...
def collect_all_passes(db, cycles, num_passes, cycle_start_map, pass1_query=None):
    ...
    for pass_number in range(1, num_passes + 1):
        ...
        if pass_number == 1 and pass1_query is not None:
            ...
            lon, lat, time, cycnums = query_pass_stacked(db, cycles, pass_number)

        rec = {**get_geometry(lon[0], lat[0]),
...
def main() -> None:
    ...
    Reads geometry and timing for every pass from the local L3 database over a
    list of cycles, anchors timing on pass 1, builds the dataset and writes it
    to NetCDF.
    """
