...
def load_ephemeris(
    path: pathlib.Path,
) -> tuple[float, numpy.ndarray, numpy.ndarray, numpy.ndarray, numpy.timedelta64]:
    ...
    eph = numpy.loadtxt(
        lines, delimiter=' ',
        dtype={'names':   ('time', 'longitude', 'latitude', 'height'),
               'formats': ('f8',   'f8',        'f8',       'f8')},
    )
...
def read_l3_pass_from_db(
    ...
    try:
        ...
        raise RuntimeError(f'L3 database query failed for cycle {cycle_number}, pass {pass_number}: {e}')
    
    lon_raw = data['longitude'].values   # ← conserver [0, 360] pour la line_string
...
# ── LineString: resample to [0, 360] BEFORE conversion ───────────────────
    # numpy.interp performs linear interpolation: in [-180, 180] the ±180°
    # jump creates spurious values; in [0, 360] the crossing at 180° is continuous.
    x_pts        = numpy.linspace(0.0, 1.0, NUM_POINTS)
    ls_lon_0360  = _resample(lon_raw[:, nadir_idx].astype(numpy.float64), x_orig, x_pts)
...
def compute_pass_timing(
    orbit: pyinterp.orbit.Orbit,
) -> list[dict]:
    """Compute start_time, end_time, pass_time for all passes of the cycle.
    Uses swath.time from the pyinterp ephemeris,
    in nanoseconds since the start of the cycle.
    Returns one record per pass (1 → n_passes_total),
    indexable as timing_records[pass_number - 1].
    """
    ...
    for pass_number in range(1, n_passes + 1):
        ...
        swath = pyinterp.orbit.calculate_swath(
            pass_data, half_swath=60.0, half_gap=10.0
        )
        ...
        if n_orig != NUM_LINES:
            print(f'  ! Pass {pass_number} : n_orig={n_orig} ≠ NUM_LINES={NUM_LINES}, rééchantillonnage')
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help='Path to local L3 SSH database directory (e.g., /data/SWOT_L3_LR_SSH)')
    parser.add_argument(
        '--cycle', type=int, required=True,
        help='Cycle number (e.g., 28 for CalVal, 1 for Science cycle 001)')
    parser.add_argument(
        '--ephemeris', type=pathlib.Path, required=True,
    ...
    orbit = pyinterp.orbit.calculate_orbit(height, lon, lat, time, cycle_duration)
    n_passes = orbit.passes_per_cycle()
